...
const MenuStatic = () => {
  ...
  const data = productItem.filter(
    (product) => product.id === "1677946920785",
    []
  )[0];
    ...
    <div className="h-full">
      <div className="w-full  p-4 max-w-3xl m-auto h-auto flex flex-col sm:flex-row">
        <div className="group w-full md:min-w-350 md:w-80 md:p-9 bg-slate-100 cursor-pointer">
          <img
            className="w-full  rounded group-hover:scale-125 transition-all "
            src={data?.imgURL}
          />
        </div>
        <div className="h-full sm:px-6 py-4">
          ...
          <div>
            <div className="text-green-600 flex items-center my-2">
          ...
          <div className="flex gap-4 w-full my-2">
            <button
              className="flex justify-center items-center py-1 px-5 border-2 border-solid w-full  min-w-[100px] max-w-[250px] whitespace-nowrap bg-green-600 text-white hover:bg-white hover:text-black font-semibold rounded"
              onClick={handleBuyProduct}
            >
              Buy
            </button>
            <button
              className="flex justify-center items-center py-1 px-5 border-2 border-solid  w-full min-w-[100px] max-w-[200px] hover:bg-green-600 hover:text-white font-semibold rounded"
              onClick={handleCartProduct}
            >
              <span className="whitespace-nowrap">Add to Cart</span>
              <FaCartPlus className="text-xl text-slate-600 hover:text-green-600 cursor-pointer self-end mx-2" />
            </button>
          </div>
